...
library(DT)
library(DVLA)
datatable(smry, caption = "Example DVLA data", options = list(pageLength = 20))
```

## Prerequisites
In addition to this package, you must apply for and be granted an API key from the UK Driver Vehicle Licensing Authority. You can do so [here](https://developer-portal.driver-vehicle-licensing.api.gov.uk/apis/vehicle-enquiry-service/Register-For-VES-API.html).

## Installation

You can install the released version of DVLA from [Github](https://github.com/Daveyr/DVLA) with:
...
## Misc
To understand more about how the API works, refer to the [Vehicle Enquiry Service API documentation](https://developer-portal.driver-vehicle-licensing.api.gov.uk/apis/vehicle-enquiry-service/vehicle-enquiry-service-description.html#vehicle-enquiry-service-api). 
